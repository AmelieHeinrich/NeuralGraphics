...
class DebugPass: Pass {
    ...
    func drawLine(from: SIMD3<Float>, to: SIMD3<Float>, color: SIMD4<Float> = SIMD4(1, 1, 1, 1)) {
        ...
        vertices.append(DebugVertex(x: from.x, y: from.y, z: from.z,
                                    r: color.x, g: color.y, b: color.z, a: color.w))
        vertices.append(DebugVertex(x: to.x,   y: to.y,   z: to.z,
                                    r: color.x, g: color.y, b: color.z, a: color.w))
    }
    ...
    func drawBox(min: SIMD3<Float>, max: SIMD3<Float>, color: SIMD4<Float> = SIMD4(1, 1, 1, 1)) {
        ...
        drawLine(from: c[0], to: c[1], color: color); drawLine(from: c[1], to: c[2], color: color)
        drawLine(from: c[2], to: c[3], color: color); drawLine(from: c[3], to: c[0], color: color)
        drawLine(from: c[4], to: c[5], color: color); drawLine(from: c[5], to: c[6], color: color)
        drawLine(from: c[6], to: c[7], color: color); drawLine(from: c[7], to: c[4], color: color)
        drawLine(from: c[0], to: c[4], color: color); drawLine(from: c[1], to: c[5], color: color)
        drawLine(from: c[2], to: c[6], color: color); drawLine(from: c[3], to: c[7], color: color)
    }
    ...
    func drawSphere(center: SIMD3<Float>, radius: Float,
                    color: SIMD4<Float> = SIMD4(1, 1, 1, 1), segments: Int = 32) {
        drawCircle(center: center, normal: SIMD3(1, 0, 0), radius: radius, color: color, segments: segments)
        drawCircle(center: center, normal: SIMD3(0, 1, 0), radius: radius, color: color, segments: segments)
        drawCircle(center: center, normal: SIMD3(0, 0, 1), radius: radius, color: color, segments: segments)
    }

    // MARK: - Capsule

    /// Wireframe capsule between `start` and `end` with the given `radius`.
    func drawCapsule(start: SIMD3<Float>, end: SIMD3<Float>, radius: Float,
                     color: SIMD4<Float> = SIMD4(1, 1, 1, 1), segments: Int = 16) {
        let dir  = end - start
        let len  = length(dir)
        guard len > 0 else { drawSphere(center: start, radius: radius, color: color, segments: segments); return }
        let axis = dir / len
        ...
        drawArc(center: start, axis1: tangent,   axis2: -axis, radius: radius,
                from: 0, to: .pi, color: color, segments: halfSegs)
        drawArc(center: start, axis1: bitangent, axis2: -axis, radius: radius,
                from: 0, to: .pi, color: color, segments: halfSegs)
        drawArc(center: end,   axis1: tangent,   axis2:  axis, radius: radius,
                from: 0, to: .pi, color: color, segments: halfSegs)
        drawArc(center: end,   axis1: bitangent, axis2:  axis, radius: radius,
                from: 0, to: .pi, color: color, segments: halfSegs)
    }
    ...
    func drawFrustum(viewProjectionInverse: simd_float4x4,
        ...
        let nbl = unproject(SIMD4(-1, -1, -1, 1)); let nbr = unproject(SIMD4( 1, -1, -1, 1))
        let ntr = unproject(SIMD4( 1,  1, -1, 1)); let ntl = unproject(SIMD4(-1,  1, -1, 1))
        let fbl = unproject(SIMD4(-1, -1,  1, 1)); let fbr = unproject(SIMD4( 1, -1,  1, 1))
        let ftr = unproject(SIMD4( 1,  1,  1, 1)); let ftl = unproject(SIMD4(-1,  1,  1, 1))

        // Near plane
        drawLine(from: nbl, to: nbr, color: color); drawLine(from: nbr, to: ntr, color: color)
        drawLine(from: ntr, to: ntl, color: color); drawLine(from: ntl, to: nbl, color: color)
        // Far plane
        drawLine(from: fbl, to: fbr, color: color); drawLine(from: fbr, to: ftr, color: color)
        drawLine(from: ftr, to: ftl, color: color); drawLine(from: ftl, to: fbl, color: color)
        // Connecting edges
        drawLine(from: nbl, to: fbl, color: color); drawLine(from: nbr, to: fbr, color: color)
        drawLine(from: ntr, to: ftr, color: color); drawLine(from: ntl, to: ftl, color: color)
    }
    ...
    func drawRing(center: SIMD3<Float>, normal: SIMD3<Float>, radius: Float,
                  color: SIMD4<Float> = SIMD4(1, 1, 1, 1), segments: Int = 32) {
    ...
    func drawQuad(_ v0: SIMD3<Float>, _ v1: SIMD3<Float>,
                  _ v2: SIMD3<Float>, _ v3: SIMD3<Float>,
                  color: SIMD4<Float> = SIMD4(1, 1, 1, 1)) {
        drawLine(from: v0, to: v1, color: color); drawLine(from: v1, to: v2, color: color)
        drawLine(from: v2, to: v3, color: color); drawLine(from: v3, to: v0, color: color)
    }

    // MARK: - Cone

    /// Wireframe cone from `tip` to a circular base centred at `base`.
    func drawCone(tip: SIMD3<Float>, base: SIMD3<Float>, radius: Float,
                  color: SIMD4<Float> = SIMD4(1, 1, 1, 1), segments: Int = 16) {
        let dir = base - tip
        let len = length(dir)
        guard len > 0 else { return }
    ...
    func drawCylinder(start: SIMD3<Float>, end: SIMD3<Float>, radius: Float,
                      color: SIMD4<Float> = SIMD4(1, 1, 1, 1), segments: Int = 16) {
        let dir = end - start
        let len = length(dir)
        guard len > 0 else { return }
    ...
    private func basisForNormal(_ normal: SIMD3<Float>) -> (SIMD3<Float>, SIMD3<Float>) {
        let up: SIMD3<Float> = abs(dot(normal, SIMD3(0, 1, 0))) < 0.99
    ...
    private func drawCircle(center: SIMD3<Float>, normal: SIMD3<Float>, radius: Float,
                             color: SIMD4<Float>, segments: Int) {
        let (t, b) = basisForNormal(normal)
        let step   = 2.0 * Float.pi / Float(segments)
        for i in 0..<segments {
            let a0 = Float(i) * step;  let a1 = Float(i + 1) * step
    ...
    private func drawArc(center: SIMD3<Float>, axis1: SIMD3<Float>, axis2: SIMD3<Float>,
