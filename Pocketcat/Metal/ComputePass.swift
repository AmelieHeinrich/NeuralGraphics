...
class ComputePass {
    ...
    func setBytes(allocator: GPULinearAllocator, index: Int, bytes: UnsafeRawPointer, size: Int) {
        ...
        RendererData.computeTable.setAddress(
    ...
    func buildBLAS(blas: BLAS) {
        self.encoder.build(
            destinationAccelerationStructure: blas.accelerationStructure,
            descriptor: blas.descriptor,
            scratchBuffer: MTL4BufferRangeMake(
                blas.scratchBuffer.getAddress(), UInt64(blas.scratchBuffer.size)))
    }
